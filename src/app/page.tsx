...
function SimulationPage() {
  ...
  /* SHARED REF for all recursive setTimeout effects — every timeout-tick
     writes into this single ref so cleanup functions always clear the
     *current* timeout even when effects re-run out of order. */
  ...
  /* ---------- FLASH & CONFETTI ---------- */
  const flash = useCallback((col = "#fff") => {
  ...
  const updateScore = useCallback((d: number) => setScore(prev => Math.max(0, prev + d)), []);

  /* ---------- HAGGLE ---------- */
  /* resolveHaggle MUST be defined BEFORE doHaggle so it can be referenced
     in the setTimeout inside doHaggle without a forward-reference error. */
  const resolveHaggle = useCallback(() => {
    ...
    if (outcome === "win") {
      ...
      addTimelineEvent("Haggle Win", "Haggle Won");
      ...
      updateScore(15); addTimelineEvent("Draw", "Draw");
  ...
  }, [addTimelineEvent, addChat, addFeed, addDecision, updateScore]);

  const doHaggle = useCallback(() => {
  ...
  }, [haggleInProgress, hagglePower, addDecision, addFeed, resolveHaggle]);
  ...
  const doDefend = useCallback(() => {
    ...
    addChat("ceo", haggleResponses[Math.floor(Math.random() * haggleResponses.length)].text);
  }, [flash, addDecision, addFeed, addChat, updateScore]);
  ...
  const doApprove = useCallback(() => {
    ...
    addChat("exec", "Exec_Mocha42: I STILL dont approve, but numbers look better...");
  }, [flash, addDecision, addFeed, addChat, updateScore]);
  ...
  const doConcede = useCallback(() => {
    ...
    updateScore(-50); addDecision("reject", "CONCEDED - Major damage!");
    addFeed("ALERT", "DR. KHUMALO CONCEDES! Board shaken.", "bad");
    addChat("troll", "BOW BEFORE THE MEME!");
  }, [flash, launchConfetti, addDecision, addFeed, addChat, updateScore]);

  const doTroll = useCallback(() => {
    flash("#ff2d95");
    if (counterAttacks > 0) {
  ...
  }, [flash, counterAttacks, addChat, addFeed, updateScore]);
  ...
  }, [flash, coffeeQuality, addDecision, addFeed, updateScore]);

  const spiceCoffee = useCallback(() => {
    flash("#ffaa00");
    ...
    updateScore(5); addDecision("haggle", "SPICE MODE!");
    addFeed("Alert", "SPICE MODE activated! Things are getting chaotic.", "pop");
    addChat("troll", "Oh you did NOT just spice that coffee!");
  }, [flash, addDecision, addFeed, addChat, updateScore]);

  const callQuorumVote = useCallback(() => {
    ...
    if (Math.random() > 0.4) {
      ...
      addFeed("System", `Quorum PASSED - ${votes} votes.`, "good");
      ...
      addFeed("System", "Quorum FAILED! Board in disarray.", "bad");
  ...
  const triggerEvent = useCallback(() => {
    ...
    addTimelineEvent("Event", evt.name.split(" ").pop() || "");
  }, [addFeed, addTimelineEvent, flash]);
  ...
  const startCrisis = useCallback(() => {
    ...
    const c = [
      ...
      { name: "Viral Thread", desc: "Thread exposes CEO coffee order history.", ti: -12, qi: -5 },
    ...
    addTimelineEvent("Crisis", c.name);
    setTimeout(() => { setCrisisActive(false); addFeed("System", "Crisis deactivated.", "sys"); }, 3000);
  }, [crisisActive, flash, launchConfetti, addFeed, addChat, addTimelineEvent]);
  ...
  const advanceCycle = useCallback(() => {
    ...
    addTimelineEvent("Cycle", `C${newCycle}D${newCycle < cycle ? day + 1 : day}`);
    if (Math.random() > 0.7 && counterAttacks < 5) { setCounterAttacks(v => v + 1); addFeed("System", "Counter-attack restored!", "good"); }
  }, [cycle, day, counterAttacks, addFeed, addTimelineEvent]);
  ...
  const resetSimulation = useCallback(() => {
    ...
    addTimelineEvent("Reset", "Reset");
  }, [addFeed, addChat, addTimelineEvent]);
  ...
  /* Bot chatter — cleanup clears timeout on effect re-run */
  useEffect(() => {
    const tick = () => {
      if (Math.random() > 0.4) {
        const msgs = [
          ...
          { who: "ceo" as const, text: "Dr. K.: Stay focused. We will get through this." },
          { who: "troll" as const, text: "TROLL_BOT_01: Just wait until they see the latte art." },
        ];
        const m = msgs[Math.floor(Math.random() * msgs.length)];
        addChat(m.who, m.text);
      }
      timeoutRef.current = setTimeout(tick, (Math.random() * 8000) + 5000);
    };
    timeoutRef.current = setTimeout(tick, 5000);
    return () => { if (timeoutRef.current) clearTimeout(timeoutRef.current); };
  }, [addChat]);

  /* Auto feed — cleanup clears timeout on effect re-run */
  useEffect(() => {
    const tick = () => {
      if (Math.random() > 0.5) {
      ...
      timeoutRef.current = setTimeout(tick, (Math.random() * 12000) + 8000);
    };
    timeoutRef.current = setTimeout(tick, 3000);
    return () => { if (timeoutRef.current) clearTimeout(timeoutRef.current); };
  }, [addFeed]);

  /* Auto stat drift — cleanup clears timeout on effect re-run */
  useEffect(() => {
    const tick = () => {
  ...
  /* Auto haggle — doHaggle is now useCallback (stable ref), so this only re-runs
     when haggleInProgress actually changes, not every render */
  useEffect(() => {
    const tick = () => {
      ...
      timeoutRef.current = setTimeout(tick, 15000 + Math.random() * 20000);
    };
    timeoutRef.current = setTimeout(tick, 12000);
    return () => { if (timeoutRef.current) clearTimeout(timeoutRef.current); };
  }, [haggleInProgress, doHaggle, addFeed]);
  ...
  const sendChat = useCallback(() => {
    ...
    setTimeout(() => {
      const replies = [
        ...
        "Dr. K.: Let us circle back on that.",
      ];
    ...
    <div className="relative min-h-screen bg-neutral-950 overflow-x-hidden">
      ...
      <div className="sim-grid grid grid-cols-1 lg:grid-cols-12 gap-4 p-4 max-w-[1600px] mx-auto">
        ...
        <section className="lg:col-span-5 sim-panel rounded-2xl p-5 relative" style={{ borderColor: "var(--neon-pink)" }}>
          ...
          <div className="panel-title text-[10px] uppercase tracking-[2px] text-neutral-500 flex items-center gap-2 mb-4">Haggle Arena</div>
          ...
          <div className="haggle-box rounded-xl p-4 text-center mb-3">
            ...
            <div className="text-[10px] text-neutral-400 leading-relaxed mb-2">
              TROLL_BOT_01 exposed a meme about CEO coffee approval.
          ...
          {/* Result */}
          {haggleResult === "win" && <div className="haggle-result haggle-result-win show p-3 rounded-xl mb-3 text-sm font-semibold">HAGGLE WON! Dr. Khumalo defends coffee policy. Board acknowledges.</div>}
          {haggleResult === "draw" && <div className="haggle-result haggle-result-draw show p-3 rounded-xl mb-3 text-sm font-semibold">DRAW! Committee will review the coffee blend next cycle.</div>}
          {haggleResult === "lose" && <div className="haggle-result haggle-result-lose show p-3 rounded-xl mb-3 text-sm font-semibold">HAGGLE LOST! Troll meme went viral. Board pressured Dr. Khumalo.</div>}
          ...
          <div className="grid grid-cols-2 gap-2 mb-2">
            <button className="big-btn btn-haggle" onClick={doHaggle}>HAGGLE</button>
            <button className="big-btn btn-defend" onClick={doDefend}>DEFEND</button>
          ...
          <div className="flex gap-2">
            ...
            <button className="big-btn btn-concede flex-1 text-[9px] py-2" onClick={doConcede}>CONCEDE</button>
          </div>
        </section>
        ...
        <section className="lg:col-span-2 sim-panel panel-orange rounded-2xl p-4 relative">
          ...
          <button className="big-btn btn-spice w-full text-[10px] py-2" onClick={spiceCoffee}>SPICE (Troll Mode)</button>
        </section>
        ...
        <section className="lg:col-span-2 sim-panel panel-green rounded-2xl p-4 relative">
          ...
          <div className="text-center mb-2">
            <span id="quorumStatus" className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[9px] font-bold ${quorum >= 60 ? "bg-success/15 text-success" : quorum >= 40 ? "bg-warning/15 text-warning" : "bg-danger/15 text-danger"}`}>
              {quorum >= 60 ? "QUORUM OK" : quorum >= 40 ? "QUORUM AT RISK" : "QUORUM LOST"}
            </span>
          </div>
          <div className="text-[10px] text-neutral-400 leading-relaxed mb-2">
          ...
          <div className="text-[9px] text-warning bg-warning/10 rounded-lg p-2 mb-3">Below 50% quorum = Troll escalation!</div>
          <button className="big-btn btn-defend w-full text-[10px] py-2.5" onClick={callQuorumVote}>Call Quorum Vote</button>
        </section>
          ...
          <div className="decision-list pr-1">
            {decisions.map(d => (
              <div key={d.id} className="flex items-center gap-2 py-1.5 border-b border-white/5 text-[11px]">
                <span className={`decision-badge decision-badge-${d.type}`}>{d.type === "approve" ? "APPROVE" : d.type === "reject" ? "REJECT" : d.type === "haggle" ? "HAGGLE" : d.type === "defend" ? "DEFEND" : d.type === "vote" ? "VOTE" : d.type === "crisis" ? "CRISIS" : "TICK"}</span>
        ...
        <section className="lg:col-span-2 sim-panel panel-orange rounded-2xl p-4 relative">
          <div className="panel-border-top panel-orange absolute top-0 inset-x-0" />
          <div className="panel-title text-[10px] uppercase tracking-[2px] text-neutral-500 flex items-center gap-2 mb-3">Controls</div>
        ...
        <section className="lg:col-span-12 sim-panel panel-pink rounded-2xl p-4 relative">
          <div className="panel-border-top panel-pink absolute top-0 inset-x-0" />
          <div className="panel-title text-[10px] uppercase tracking-[2px] text-neutral-500 flex items-center gap-2 mb-3">Event Timeline</div>
...
/* ================================================================
    ...
    ================================================================ */
const feedMessages = [
  { text: "SynthCorp filed formal complaint about executive coffee blend", type: "bad", actor: "Corp Secretary" },
  ...
  { text: "Secret ballot initiated: Should CEO maintain coffee decision?", type: "sys", actor: "Board Vote" },
  { text: "TROLL_BOT_01 sequel meme: CEO espresso shot of power", type: "pop", actor: "TROLL_BOT_01" },
...
const haggleResponses = [
  { text: 'My coffee approval was within full regulatory compliance. The quorum issue lies with those who cant handle espresso.' },
  { text: 'I invoke Section 14.7 - the blend meets all standards.' },
  { text: 'Perhaps we should water down the coffee AND the drama.' },
  { text: 'A taste-test tribunal. Let the board judge the blend.' },
  { text: 'TROLL_BOT_01, your meme had worse engagement than my quarterly report.' },
  { text: 'I will switch to decaf if you maintain quorum without caffeine. Deal?' },
  { text: '87% board trust says you are all just hangry. Next item.' },
];

const trollQuotes = [
  "The people demand answers about the coffee!",
  "This meme has 10K likes. You are going viral, CEO!",
  "COFFEE-GATE: Episode III - The Latte Strikes Back",
  "Quorum update: 3 more members heading to the espresso machine",
  "I am not blocking you, CEO. You are blocking yourself.",
];

const coffeeNames = [
  "Espresso Supreme", "Dark Roast Decadence", "Lavender Latte",
  "Dragon Breath Blend", "Diamond Dust Cold Brew", "Nitro Cascade",
  "Mocha Mayhem", "Ghost Pepper Grande",
];